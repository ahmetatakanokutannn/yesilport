...
function CoursesPage() {
  return <><section className="section section-muted"><div className="container"><HeaderBlock title="Eğitimler" subtitle="CBAM, CSRD ve yeşil lojistik süreçleri için Google Drive videolarıyla beslenen statik dersler." /><div className="grid-auto">{courses.map((course) => <Link to={`/egitimler/${course.id}`} className="card p-6 hover:shadow-xl hover:-translate-y-1 transition" key={course.id}><span className="pill bg-emerald-50 text-emerald-700">{course.level}</span><h3 className="font-black text-xl mt-4">{course.title}</h3><p className="text-slate-600 mt-3">{course.summary}</p><p className="text-sm text-slate-500 font-bold mt-5">{course.lessons.length} ders</p><span className="inline-flex items-center gap-1 text-emerald-700 font-black mt-5">Derse Git <ChevronRight size={18} /></span></Link>)}</div></div></section><Footer /></>;
}
...
function CourseDetailPage() {
  ...
  return <><section className="section section-muted"><div className="container"><HeaderBlock eyebrow={course.level} title={course.title} subtitle={course.summary} /><div className="grid lg:grid-cols-[1fr_320px] gap-6 items-start"><div className="card p-5"><iframe src={activeLesson.videoUrl} className="w-full aspect-video rounded-2xl" allow="autoplay" allowFullScreen title={activeLesson.title} /><h2 className="font-black text-2xl mt-6">{activeLesson.title}</h2>{activeLesson.duration && <p className="text-slate-600 mt-2">{activeLesson.duration}</p>}</div><aside className="card p-5"><h3 className="font-black text-xl mb-4">Dersler</h3><div className="grid gap-2">{course.lessons.map((lesson) => <button key={lesson.id} type="button" onClick={() => setActiveLessonId(lesson.id)} className={`text-left rounded-xl border p-4 font-bold transition ${lesson.id === activeLesson.id ? "border-emerald-500 bg-emerald-50 text-emerald-800" : "border-slate-200 hover:border-emerald-300"}`}><span>{lesson.title}</span>{lesson.duration && <span className="block text-sm text-slate-500 mt-1">{lesson.duration}</span>}</button>)}</div></aside></div></div></section><Footer /></>;
}
