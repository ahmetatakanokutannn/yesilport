...
async function seed() {
    ...
    for (const c of courses) {
        const result = await db_1.pool.query("INSERT INTO courses (title,description,category,total_modules,duration_hours,access_level,thumbnail_url) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING RETURNING id", [...c, "https://images.unsplash.com/photo-1494412519320-aa613dfb7738?auto=format&fit=crop&w=900&q=80"]);
        const courseId = result.rows[0]?.id;
        if (courseId) {
            await db_1.pool.query("INSERT INTO course_modules (course_id,title,video_url,transcript,order_index,duration_minutes) VALUES ($1,$2,$3,$4,1,35)", [
                courseId,
                "Giriş ve kavramlar",
    ...
    for (const [title, slug, category, date] of posts) {
        await db_1.pool.query(`INSERT INTO blog_posts (title,slug,excerpt,content,category,tags,thumbnail_url,author_id,published,published_at)
    ...
    for (const [title, category, severity] of alerts) {
        await db_1.pool.query("INSERT INTO regulation_alerts (title,content,category,severity) VALUES ($1,$2,$3,$4)", [title, "YeşilPORT mevzuat merkezi bu değişiklik için aksiyon planı önerir.", category, severity]);
    }
}
seed()
